...
# Usage: ./sign_and_verify.sh <path-to-pdf>
...
# ── 1. Login ──────────────────────────────────────────────────────────────────
echo "[1/4] Logging in..."
TOKEN=$(curl -s --cacert "$CA" -X POST "$BASE/api/auth/login" \
...
SIGN_RESPONSE=$(curl -s --cacert "$CA" -X POST "$BASE/api/documents/sign" \
...
PUBLIC_KEY=$(curl -s --cacert "$CA" "$BASE/api/documents/public-key" \
  -H "Authorization: Bearer $TOKEN" \
